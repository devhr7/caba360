...
const edit = (prod) => {
    ...
    router.get(route("CumpLaborCampo.edit", reg_datatable.value.id)); // redirecciona a la vista Editar
};

// boton de Crear
function crear() {
    router.get(route("CumpLaborCampo.crear")); // Redirecciona a la Vista Crear
}
...
function verificar() {
    ...
    form.post(route("CumpLaborCampo.verificar"), {
        preserveScroll: true,
        onSuccess: () => {
...
const op = ref();
const datatable_detalle_cuadrilla = ref({});

const displayRecord = (event, prod) => {
    op.value.hide();
    datatable_detalle_cuadrilla.value = { ...prod };
    nextTick(() => {
        op.value.show(event);
    });
};
        ...
        <div class="py-12">
            <div class="mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    >
                        ...
                        >
                            ...
                            <Column :exportable="false"  header="Cuadrilla"  style="max-width: 3rem">
                            <!-- Boton Editar -->
              ...
              <Dialog
                        ...
                        modal
                        header="Verificar Cumplido Labor Campo"
                        :style="{ width: '25rem' }"
                    >
                 ...
                 <Popover ref="op">
                        <div
                            ...
                            v-if="datatable_detalle_cuadrilla"
                        >
                            <div>
                                <span class="font-bold">Cuadrilla: </span>
                                <ul>
                                    <li v-for="(member, index) in datatable_detalle_cuadrilla.cuadrilla.split(',')" :key="index">
                                        {{ member.trim() }}
                                    </li>
                                </ul>
                                <span class="font-bold">Total: </span>
                                {{ datatable_detalle_cuadrilla.cuadrilla.split(',').length }}
